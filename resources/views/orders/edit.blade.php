...
@section('page-title')
  @lang('orders.edit.title', ['id' => $order->id])
@stop

@section('section-title')
  @lang('orders.edit.section-title', ['id' => $order->id])
@stop
...
@section('page-content')
  ...
  <div class="row">
    <form action="{{ action('OrderController@update', ['order' => $order->id]) }}" method="POST" class="form-horizontal form-label-left" novalidate>
      {{ csrf_field() }}
      {{ method_field('PATCH') }}
      <!--left section-->
          ...
          <div id="items_container" class="row">
            ...
            <div class="form-group">
              ...
              <div class="col-md-4 col-sm-4 col-xs-12">
                <input type="text" value="{{ $orderDetail->product->name }} - {{ $orderDetail->product->price }}@lang('common.usa_currency_label')" required="required" class="product form-control col-md-7 col-xs-12" disabled/>
              </div>
              <label class="control-label col-md-2 col-sm-2 col-xs-12" for="first-name"> @lang('orders.common.label_amount') : </label>
              <div class="col-md-2 col-sm-2 col-xs-12">
                <input type="number" value="{{ $orderDetail->amount }}" class="amount form-control col-md-7 col-xs-12 amount-box" disabled>
              </div>
      ...
      <div class="col-md-5 col-sm-5 col-xs-12">
        ...
        <div class="form-group">
            ...
            <div class="input-group col-md-12 col-sm-12 col-xs-12">
                <select class="form-control" autofocus="" name="supplier_id">
                    ...
                    <option value="{{ $supplier->id }}" {{ $supplier->id == $order->supplier_id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="input-group col-md-12 col-sm-12 col-xs-12">
                <input type="number" name="total_cost" value="{{ $order->total_cost }}" class="form-control">
        ...
        <div class="form-group">
            ...
            <div class="input-group col-md-12 col-sm-12 col-xs-12">
                <textarea class="form-control" name="description" row=3>{{ $order->description }}</textarea>
            </div>
        </div>
        <div class="form-group">
            <div class="col-md-3 col-sm-3 col-xs-12 col-md-offset-3">
              <a href="{{ action('OrderController@edit', ['order' => $order->id]) }}" class="btn btn-danger"><i class="fa fa-refresh"></i> @lang('orders.common.btn_reset')</a>
            </div>
...
@push('end-page-scripts')
  ...
  <script src="/js/orders/edit.js" type="text/javascript"></script>
@endpush
